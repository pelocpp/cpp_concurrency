...
static void test_event_loop_00()
{
...
static void test_event_loop_01()
{
    ...
    eventLoop.enqueue(
        [] () { Logger::log(std::cout, "Message from a different thread"); }
    );
...
static void anotherWorker()
{
    Logger::log(std::cout, "AnotherWorker starting ...");
    std::thread::id tid{ std::this_thread::get_id() };
    Logger::log(std::cout, "AnotherWorkers TID: ", tid);
    Logger::log(std::cout, "AnotherWorker done.");
}
...
static void test_event_loop_02()
{
    ...
    for (size_t i{}; i != 5; ++i) {
        ...
        std::function<void()> aw{ anotherWorker };
        eventLoop.enqueue(std::move(aw));
