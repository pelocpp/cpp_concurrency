...
parallel oder parallel und vektorisiert ausgeführt wird.


Im Detail ist der *Execution Policy* Parameter wie folgt definiert:

  * `sequenced_policy` - legt fest, dass die Ausführung eines möglicherweise parallelisierbaren Algorithmus nicht parallelisiert wird.</br>
     Das entsprechende globale Objekt ist `std::execution::seq`.
  * `parallel_policy` - legt fest, dass die Ausführung eines möglicherweise parallelisierbaren Algorithmus parallel ausgeführt werden soll.</br>
     Das entsprechende globale Objekt ist `std::execution::par`.
  * `parallel_unsequenced_policy` - legt fest, dass die Ausführung eines parallelisierbaren Algorithmus parallelisiert und vektorisiert ausgeführt werden soll.</br>
